...
When proprietary families cannot be licensed, **Sohne** or **Tiempos Headline** stand in for Domaine Display, and **Geist** or **Inter Tight** can replace ABC Favorit. Inter and Geist Mono are open-source and should be used directly.
...
When Domaine Display is unavailable, clamp `lineHeight` to 1.0 explicitly and apply `font-feature-settings: "ss01", "liga"` on the substitute serif to mimic the alternate glyphs. Sohne or Tiempos Headline will read closest. ABC Favorit substitutes (Geist, Inter Tight) typically default to looser tracking - apply -0.5% letter-spacing on body sizes to compensate.
...
- Background `{colors.surface-card}` placeholder, `rounded: {rounded.full}`, 32x32px.
