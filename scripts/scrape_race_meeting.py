...
def main() -> None:
    parser = argparse.ArgumentParser(description="局部爬取香港马会赛程 / 赛日总览页面。")
    ...
    url = client.race_card_url(args.race_date, args.racecourse)
    identity = f"{args.race_date}_{args.racecourse}"
    result = client.scrape_url("race_meetings", url, identity)
