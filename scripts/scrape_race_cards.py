...
def main() -> None:
    parser = argparse.ArgumentParser(description="局部爬取香港马会排位/参赛马页面。")
    ...
    url = client.race_card_url(args.race_date, args.racecourse, args.race_no)
    identity = f"{args.race_date}_{args.racecourse}_R{args.race_no or 'default'}"
    result = client.scrape_url("race_cards", url, identity)
